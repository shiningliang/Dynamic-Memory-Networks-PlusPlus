...
class EpisodicMemory(nn.Module):
    ...
    def forward(self, facts, questions, prevM):
        ...
        '''
        G = self.make_interaction(facts, questions, prevM)  # Gate Attention
        C = self.AGRU(facts, G)
...
class QuestionModule(nn.Module):
    def __init__(self, vocab_size, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.transformer_encoder = Encoder(4, 2, self.hidden_size, 0.2)
    ...
    def forward(self, questions, word_embedding):
        ...
        gru() -> (1, #batch, #hidden)
        '''
        mask = (questions != 0).unsqueeze(-2)
        questions = word_embedding(questions)
        # _, questions = self.gru(questions)
        # questions = questions.transpose(0, 1)
        questions = self.transformer_encoder(questions, mask)
        questions = questions[:, -1, :].unsqueeze(-2)
        return questions
...
class InputModule(nn.Module):
    ...
    def forward(self, contexts, word_embedding):
        ...
        mask = torch.ones(batch_num, 1, sen_num, dtype=torch.int32).cuda()
        ...
        facts = self.transformer_encoder(contexts, mask)
        return facts
